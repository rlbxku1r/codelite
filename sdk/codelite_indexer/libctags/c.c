...
static void parseGeneralToken (statementInfo *const st, const int c)
{
    ...
    if (isident1 (c) || (isLanguage (Lang_java) && isHighChar (c))) {
        ...
        if ( isType (st->context, TOKEN_NAME) && // context name
            (isType (activeToken (st), TOKEN_NAME) || (isType (activeToken (st), TOKEN_KEYWORD) && activeToken(st)->keyword == KEYWORD_CONST)) && // D::E foo or D::E const
